    ...
    describe('register', () => {
      it('returns the correct type and payload', () => {
        expect(register({ expected: true })).toEqual(
          { type: REGISTER, payload: { expected: true } }
        )
      })
    })
  })
  ...
  describe('Reducer', () => {
    ...
    describe('REGISTER_SUCCESS', () => {
      it('returns the auth object', () => {
        expect(
          reducer({}, { type: REGISTER_SUCCESS, payload: { mnemonic: 'mnemonic' } })
        ).toEqual(
          { mnemonic: 'mnemonic', requestPassword: false, mnemonicError: false, derivationError: false }
        )
      })
    })
  })
